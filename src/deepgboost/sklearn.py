...
class DeepGBoostRegressor(BaseEstimator, RegressorMixin):
    ...
    def fit(
        ...
        *,
        ...
        if eval_set:
            raw_evals = [
                (np.asarray(Xv, dtype=np.float64), np.asarray(yv, dtype=np.float64).ravel(), f"eval_{i}")
                for i, (Xv, yv) in enumerate(eval_set)
            ]
            if self.early_stopping_rounds is not None:
                from .callback import EarlyStopping
                all_callbacks.append(EarlyStopping(patience=self.early_stopping_rounds))
        ...
        *,
        ...
        if n_classes == 2:
            ...
            for k in range(n_classes):
                ...
                if eval_set:
                    eval_set_k = [
                        (Xv, (LabelEncoder().fit_transform(yv) == k).astype(np.float64))
                        for Xv, yv in eval_set
                    ]
                model_k = self._fit_binary(X, y_k, eval_set_k, all_callbacks, model_kw)
